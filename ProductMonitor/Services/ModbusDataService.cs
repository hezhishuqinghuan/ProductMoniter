                    ...
                    using (var serialPort = new SerialPort(_portName, _baudRate, Parity.None, 8, StopBits.One))
                    {
                        ...
                        for (int i = 0; i < 7; i++)
                        {
                            ...
                            switch (i)
                            {
                                ...
                                case 2: fluctuation = (int)(3 * Math.Sin(readIndex / 5.0)); baseVal = Math.Max(20, Math.Min(35, baseVal + fluctuation)); break;
                                case 3: fluctuation = (int)(10 * Math.Sin(readIndex / 8.0)); baseVal = Math.Max(40, Math.Min(80, baseVal + fluctuation)); break;
                                case 4: fluctuation = (int)(20 * Math.Sin(readIndex / 6.0)); baseVal = Math.Max(10, Math.Min(100, baseVal + fluctuation)); break;
                        ...
                        for (int i = 7; i < 14; i++)
                        {
                            ...
                            switch (i - 7) //映射为0-6
                            {
                                ...
                                case 1: fluctuation = (int)(10 * Math.Sin(readIndex / 11.0)); baseVal = Math.Max(200, Math.Min(240, baseVal + fluctuation)); break; // 电压
                                case 2: fluctuation = (int)(50 * Math.Sin(readIndex / 13.0)); baseVal = Math.Max(800, Math.Min(1200, baseVal + fluctuation)); break; // 转速
